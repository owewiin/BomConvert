...
#pragma warning restore CS8600
        ...
        private void ReadExcelBom(string fullPath)
        {
                ...
                try
                {
                    ...
                    for (int i = 5; i < table.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            {
                                IsAltLine = false,
                                PartName = GetFinalPartName(GetSafeStringValue(row, 1, "零件名稱"), partNumber),  // 只修改這一行
                                Spec = GetSafeStringValue(row, 2),                    // 規格保持原樣
                                Package = GetSafeStringValue(row, 3, "包裝"),         // 會返回 "0805"
                                Quantity = qty,
                                Code = partNumber,
                                UnitPrice = price,
            ...
            "ESR", "Low", "High", "Temp", "V", "uF", "nF", "pF" }
                    ...
                    string CleanSpecString(string spec) =>
                        Regex.Replace(spec.Trim(' ', '-', '/', ':', '：', '，', ';', '；', '|', '\\', '　'), @"\s+", " ").Trim();
        ...
        private string GetBestPackageValue(IEnumerable<string?> packages)
        {
            ...
            var smdSizes = new[] { "0603", "0805", "1206", "1210", "1812", "2512", "2728", "3920" };
        ...
        private List<BomPreviewItem> MergeDuplicateItems(List<BomPreviewItem> items)
        {
            ...
            foreach (var group in groupedItems)
            {
                ...
                else
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"[INFO] 合併零件: {firstItem.PartName} ({firstItem.Spec}) - 合併了 {group.Count()} 項，總數量: {mergedItem.Quantity}");
                }
            }
        ...
        private string GetCleanComponentName(string[] values)
        {
            ...
            // 按優先級檢查元件類型 - 先檢查更具體的類型
            foreach (var mapping in componentMappings.OrderByDescending(x => x.Value.Length))
            {
        ...
        {
            string[] smdSizes = { "0603", "0805", "1206", "1210", "1812", "2512", "2728", "3920" };
...
private bool IsInvalidValue(string value, string[] invalidPatterns)
{
    return invalidPatterns.Any(pattern => 
        ...
        {
            string[] sizeOrder = { "0603", "0805", "1206", "1210", "1812", "2512", "2728", "3920" };
        ...
        {
            var match = Regex.Match(spec, @"\b(0603|0805|1206|1210|1812|2512|2728|3920)\b");
            ...
            var columnStyles = new Dictionary<int, (int fontSize, XLAlignmentHorizontalValues horizontal, bool wrapText)>
    {
        ...
        // 新增：LostFocus 事件處理器（當用戶輸入完成並離開焦點時觸發）
        ...
        private void LoadStaffSettings()
        {
            try
            {
                SelectedRDAssistant = Microsoft.Win32.Registry.GetValue(
                    @"HKEY_CURRENT_USER\Software\SchBomConvert", "RDAssistant", "Fish")?.ToString() ?? "Fish";
                SelectedLayoutPerson = Microsoft.Win32.Registry.GetValue(
                    @"HKEY_CURRENT_USER\Software\SchBomConvert", "LayoutPerson", "WEI")?.ToString() ?? "WEI";
                SelectedCircuitDesigner = Microsoft.Win32.Registry.GetValue(
                    @"HKEY_CURRENT_USER\Software\SchBomConvert", "CircuitDesigner", "LSP")?.ToString() ?? "LSP";

                // 延遲設定，確保 UI 已載入
                this.Loaded += (sender, e) =>
                {
                    if (RDAssistantComboBox != null) RDAssistantComboBox.Text = SelectedRDAssistant;
                    if (LayoutPersonComboBox != null) LayoutPersonComboBox.Text = SelectedLayoutPerson;
                    if (CircuitDesignerComboBox != null) CircuitDesignerComboBox.Text = SelectedCircuitDesigner;
